...
let university = [];
  ...
  graduates.filter(item => university.push(item.university));
  university.sort();
...
const contactInfo = graduates.map(students => students.first_name + " " + students.email);
...
const unisWithUni = []
graduates.map(uniName => {
  if(uniName.university.includes("Uni")) {
    return unisWithUni.push(uniName);
  } 
})
...
*/
  ...
  const displayNames = [];
  zooAnimals.forEach(zooAnim => {
    return displayNames.push(`Name: ${zooAnim.animal_name}, Scientific: ${zooAnim.scientific_name}`)
  });
...
*/
const lowCaseAnimalNames = zooAnimals.map(currentValue => {
